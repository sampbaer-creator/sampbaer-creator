# 👋 Hello, my name is Samuel.

I am an **Information Systems student, business intelligence analyst, and software developer** focused on turning complex information into clear reports, useful applications, and approachable user experiences.

Here you will find:

- 📊 Business intelligence dashboards and reporting projects
- 📈 Statistical analysis, forecasting, and data visualization
- 💻 Responsive web and cross-platform applications
- 🔌 Projects that use live APIs, databases, and public data
- 📝 Documentation covering architecture, decisions, and lessons learned

### Connect with me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Samuel_Baer-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/samuel-p-baer/)
[![Portfolio](https://img.shields.io/badge/Portfolio-013e37?style=for-the-badge&logo=googlechrome&logoColor=ffefb3)](https://sampbaer-creator.github.io/Portfolio/)
[![BearVault](https://img.shields.io/badge/BearVault-Household_Finance-315f50?style=for-the-badge&logo=vercel&logoColor=white)](https://github.com/sampbaer-creator/BaerVault)
[![GridGuard](https://img.shields.io/badge/Live_GridGuard-1f9c8b?style=for-the-badge&logo=googlemaps&logoColor=white)](https://gridguard-utah.earthy-moth-6817.chatgpt.site)
[![WattWeather](https://img.shields.io/badge/Live_WattWeather-ffefb3?style=for-the-badge&logo=githubpages&logoColor=013e37)](https://sampbaer-creator.github.io/WattWeather/)

I am open to internships and early-career opportunities in **business intelligence, data analytics, data science, and software development**.

## What I do

I build Power BI dashboards, automate recurring reporting, analyze data with SQL, Python, and R, and create applications that make technical information easier to understand.

At Utah Valley University, I am completing a **B.S. in Information Systems with an emphasis in Business Intelligence**. My experience includes supporting reporting across 10 departments, creating more than seven Power BI reports, and building portfolio applications with live data and carefully designed user experiences.

![Power BI Reports](https://img.shields.io/badge/Power_BI_reports-7%2B-013e37?style=flat-square)
![Departments](https://img.shields.io/badge/Departments_supported-10-ffefb3?style=flat-square&labelColor=013e37)
![Data Annotations](https://img.shields.io/badge/Data_annotations-5%2C000%2B-013e37?style=flat-square)
![Graduation](https://img.shields.io/badge/Graduation-April_2027-ffefb3?style=flat-square&labelColor=013e37)

## Featured projects

### 🐻 [BearVault](https://github.com/sampbaer-creator/BaerVault)

A secure household-finance application for shared budgeting, cash-flow tracking, and investment management. Clerk Organizations define household membership, Supabase row-level security isolates financial records, and Twelve Data provides live and historical market information.

**Built with:** Next.js, React, TypeScript, Clerk, Supabase PostgreSQL, Twelve Data, Mantine, Recharts, and Vercel

### 🗺️ [GridGuard](https://github.com/sampbaer-creator/GridGuard)

A Utah planning and property-intelligence platform with satellite mapping, official flood, wildfire, and wind overlays, scenario comparison, explainable risk metrics, and planning-ready exports.

**Built with:** Next.js, React, FastAPI, Python, Leaflet, GIS services, and Cloudflare Workers

**[Open the live application →](https://gridguard-utah.earthy-moth-6817.chatgpt.site)**

### ☀️ [WattWeather](https://github.com/sampbaer-creator/WattWeather)

A responsive U.S. weather app for current conditions and seven-day forecasts by city or ZIP code.

**Built with:** C#, Blazor WebAssembly, ASP.NET Core, Open-Meteo REST APIs, CSS, and xUnit

**[Open the live application →](https://sampbaer-creator.github.io/WattWeather/)**

### 🌐 [Portfolio](https://github.com/sampbaer-creator/Portfolio)

A responsive personal portfolio presenting my business intelligence work, analytics projects, professional experience, résumé, and software-development projects.

**Built with:** JavaScript, React, Tailwind CSS, animation, and responsive design

**[Visit my portfolio →](https://sampbaer-creator.github.io/Portfolio/)**

### 📊 [Business Statistics Analysis](https://github.com/sampbaer-creator/R_Final_Project)

A client-ready statistical analysis for the UVU Women’s Impact Lab using regression, hypothesis testing, exploratory analysis, and data visualization in R.

### 💍 [Our Wedding](https://github.com/sampbaer-creator/Sam_Kelcee)

A personal event website featuring custom imagery, event information, registry links, and an RSVP experience.

## Technology toolbox

### Languages, frameworks, and data tools

<p>
  <img src="https://skillicons.dev/icons?i=python,r,postgres,mysql,sqlite,js,react,nextjs,cs,dotnet,html,css,tailwind,fastapi,supabase,vercel&perline=8" alt="Python, R, PostgreSQL, MySQL, SQLite, JavaScript, React, Next.js, C sharp, .NET, HTML, CSS, Tailwind CSS, FastAPI, Supabase, and Vercel" />
</p>

### Business intelligence and analytics

![Power BI](https://img.shields.io/badge/Power_BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=013e37)
![Excel](https://img.shields.io/badge/Excel-217346?style=for-the-badge&logo=microsoftexcel&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![R Markdown](https://img.shields.io/badge/R_Markdown-276DC3?style=for-the-badge&logo=r&logoColor=white)
![Statistics](https://img.shields.io/badge/Statistical_Analysis-013e37?style=for-the-badge&logo=googleanalytics&logoColor=white)

### Cloud, GIS, and developer tools

<p>
  <img src="https://skillicons.dev/icons?i=azure,cloudflare,git,github,vscode,postman,vite&perline=7" alt="Azure, Cloudflare, Git, GitHub, Visual Studio Code, Postman, and Vite" />
</p>

![Leaflet](https://img.shields.io/badge/Leaflet-199900?style=for-the-badge&logo=leaflet&logoColor=white)
![ArcGIS](https://img.shields.io/badge/ArcGIS-2C7AC3?style=for-the-badge&logo=esri&logoColor=white)
![REST APIs](https://img.shields.io/badge/REST_APIs-009688?style=for-the-badge&logo=fastapi&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=for-the-badge&logo=githubactions&logoColor=white)
![Codex](https://img.shields.io/badge/Codex-000000?style=for-the-badge&logo=openai&logoColor=white)

## GitHub overview

<p>
  <img height="165" src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=sampbaer-creator&theme=github" alt="Samuel's GitHub statistics" />
  <img height="165" src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=sampbaer-creator&theme=github" alt="Samuel's repository languages" />
</p>

## Currently learning

- Forecasting and explainable machine learning
- Cloud-based business intelligence and Azure data services
- Accessible visualization and dashboard design
- Maintainable application architecture
- Turning technical analysis into clear business recommendations
